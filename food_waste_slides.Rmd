---
title: "Food Waste from California Farms"
author: "Warren Watts"
date: "October 3, 2018"
...
library(tidyverse)
```
##  Food Waste from California Food Production
The following slides show food waste at the retail and home level for the last 5 years of available data, 2012-2016, for California food production. The food production numbers are from *California Agricultural Statistics Review*, which is produced yearly by the California Department of Food and Agriculture, except for 2013 which was not available and was pulled from the USDA report. Yearly reports can be found at [**USDA's California Statistical Reports Website**](https://www.nass.usda.gov/Statistics_by_State/California/Publications/Annual_Statistical_Reviews/).
Food waste percentages at the retail and consumer level were pulled from the USDA's *ERS Food Availability (Per Capita) Data System (FADS)*, in particular the *Loss Adjusted Food Availability (LAFA)* tables. [**Link to FADS Webpage**](https://www.ers.usda.gov/data-products/food-availability-per-capita-data-system/).
...
theme_crop <-
  theme(axis.text.x = element_text(color="black", size=9, angle=90),
        ...
        axis.title.x = element_text(color="blue3", size=12, face="bold"),
        axis.title.y = element_text(color="coral", size=12, face="bold"))
...
cal_crop_graph %>% 
  ...
  geom_col() +
  labs(title="California Food Crop Production 2012-2016", x="Crop",
       y="Production in 1000s of tons") +
  theme_crop +
  legend_crop
```

_FM is Fresh Market, _Proc is processed (canned etc.), _Dry and _Dried are dry/dried
...
cal_crop_graph %>% 
  ...
  ggplot(aes(x=Commodity,y=Ktons,fill=Usage)) +
    ...
    labs(title="California Food Crop Production 2012-2016\n w/o processed tomatoes and wine grapes",
         x="Crop", y="Production in 1000s of tons") +
    legend_crop + 
    theme_crop
```

High volume crops removed for better detail on others
...
cal_crop_graph %>% 
  ...
  ggplot(aes(x=Commodity,y=Ktons,fill=Usage)) +
    geom_bar(stat="identity") +
    labs(title="California Nut Crop Production 2012-2016",
         x="Crop", y="Production in 1000s of tons") +
    legend_crop +
    theme_crop_sm
```
...
cal_crop_graph %>% 
  ...
  ggplot(aes(x=Commodity,y=Ktons,fill=Usage)) +
  geom_bar(stat="identity") +
  labs(title="California Grain, Sugar and Oil Crop Production 2012-2016",
       x="Crop", y="Production in 1000s of tons") +
  legend_crop +
  theme_crop_sm
...
cal_crop_graph %>% 
  ...
  ggplot(aes(x=Commodity,y=Ktons,fill=Usage)) +
  geom_bar(stat="identity") +
  labs(title="California Fruit Crop Production 2012-2016\n without wine grapes",
       x="Crop", y="Production in 1000s of tons") +
  legend_crop + 
  theme_crop
```
...
cal_crop_graph %>% 
  ...
  ggplot(aes(x=Commodity,y=Ktons,fill=Usage)) +
  geom_bar(stat="identity") +
  labs(title="California Vegetable Crop Production 2012-2016\n without processed tomatoes",
       x="Crop", y="Production in 1000s of tons") +
  legend_crop + 
  theme_crop
```
...
cal_animal_graph %>% 
  ggplot(aes(x=Commodity,y=Ktons,fill=Usage)) +
  geom_bar(stat="identity") +
  labs(title="California Animal Production 2012-2016",
...
******************
##  Cost of Producing Food from Animals vs. Plants
Food produced from animals takes more resources than plants.

The following slides show the value of food produced in California and resource costs for the last 5 years for which data is available. 

Labor and capital equipment costs were not included because the available numbers were not separable for animal and plant production.

Total costs for California in seed, fertizer, pesticides and fuel were assigned to crops, costs for purchased livestock, feed and electricity were assigned to animals. 

* Numbers from *California Agricultural Statistics Review*. 
...
##  Wasted Produce Due to Ugliness, Blemishes or Size
Produce is often rejected by retail buyers due to weird shapes, blemishes or the desire to have a standard size. 

Estimates of how much produce is wasted range from 10-15% not harvested and 20-30% rejected by retailers. 

Conservative figures of 10% unharvested and 20% rejected by retailers were used for the following slides. 
...
USDA annual crop reports were used for the US numbers and the *California Agricultural Statistics Review* was used for California numbers.
